...
# - Isolated VPC with HA (multi-AZ NAT)
# - VPC Flow Logs with longer retention
# - Strict AWS Config guardrails
...
# - NAT Gateway per AZ (HA configuration)
...
module "network" {
  ...
  single_nat_gateway = var.single_nat_gateway # false = HA for prod

  enable_flow_logs         = true
  flow_logs_retention_days = 365 # Longer retention for prod
...
# - All security rules enabled and enforced
################################################################################
